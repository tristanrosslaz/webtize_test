$(function(){
    ...
    $("#peak_time_foot_count, #lean_time_foot_count, #operation_days, #no_of_competitors,#actual_gross_sales,#lowest_price_0, #lowest_price_1,lowest_price_2,lowest_price_3,lowest_price_4").change(function(){
    ...
    $("#operation_days, #monthly_rent").change(function(){
    	var operation_days = $("#operation_days").val() == "" ? 1 : $("#operation_days").val();
    	var monthly_rent = $("#monthly_rent").val() == "" ? 0 : $("#monthly_rent").val();
    	compute_rent(monthly_rent,operation_days);
    });

    $("#labor_cost_per_crew, #recommended_crew, #working_days, #operation_days").change(function(){
    ...
    $("#operation_days, #ew").change(function(){
    	var operation_days = $("#operation_days").val() == "" ? 1 : $("#operation_days").val();
    	var ew = $("#ew").val() == "" ? 0 : $("#ew").val(); 
    	compute_electricity(ew, operation_days);
    });

    // $("#gov_permit").change(function(){
    // 	var gov_permit = $(this).val();
    // 	compute_gov_permit(gov_permit);
    // });

    $("#marketing_fee").change(function(){
    	var marketing_fee = $(this).val();
    	compute_marketing_fee(marketing_fee);
    });

    $("#other_terms, #operation_days").change(function(){
    	var operation_days = $("#operation_days").val() == "" ? 1 : $("#operation_days").val();
    	var other_fees = $("#other_terms").val() == "" ? 0 : $("#other_terms").val();
    	compute_other_fee(other_fees,operation_days);
    });

    $("#monthly_rent, #peak_time_foot_count, #lean_time_foot_count, #labor_cost_per_crew, #recommended_crew, #working_days,#ew ,#gov_permit, #marketing_fee,#other_terms, #operation_days, #other_terms").change(function(){
    ...
    $("#break_even_gp").change(function(){
     ...
     $("#bes_lowest_srp, #break_even_bes, #break_even_gp, #marketing_fee, #monthly_rent, #ew, #labor_cost_per_crew, #recommended_crew, #working_days, #operation_days").change(function(){
         var thiss = $(this).val();
