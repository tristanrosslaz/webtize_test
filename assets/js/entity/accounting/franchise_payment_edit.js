...
loadTables();
    ...
    function loadTables(){
        ...
        $.ajax({
            type: 'post',
            url: base_url+'Main_franchise_accounting/franchise_payment_edit_table',
            ...
            success:function(data){
                ...
                if(data.success == 1){
                    ...
                    try{
                        for(i=0;i<count;i++){
                            editArray = [fpnoData[i],paymentData[i],amountData[i],delBtnData[i]];
...
$("#approvePayment").click(function(e){
    $.ajax({
            url: base_url+"Main_franchise_accounting/approvePayment",
            type: 'post',
            data: { 
                'fpno':fpno
            },
            ...
            success:function(data){
                ...
                if(data.success == 1){
                    ...
                    window.setTimeout(function(){
                         window.location.href=base_url+"Main_franchise_accounting/franchise_payment_transaction_history/" + $("#token").val();
                  },1000);

                }else{
                    $.toast({
                        heading: 'Note',
                        text: 'No Payment Approve.',
                        icon: 'error',
                        loader: false,  
                        stack: false,
                        position: 'top-center', 
...
var table = $('#table-grid').DataTable({ //declaring of table
    columnDefs: [{ targets: [3], visible: true, orderable: false, sClass: 'text-center'}],
...
$(".BtnSaveProceed").click(function(e){
    ...
    //getting data in datatable
        ...
        dataID.each(function (value, index) {
            ...
            idArr = idArr.concat(value);
        });   

    //getting data in datatable
        var amtQTY = table.rows().columns(2).data();
        amtQTY.each(function (value, index) {
    ...
    if (totalamt == 0) {
        ...
        $.ajax({
            url: base_url+"Main_franchise_accounting/editPayment",
            type: 'post',
            data: { 
