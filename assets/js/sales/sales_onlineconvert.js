...
var base_url = $("body").data('base_url'); //base_url come from php functions base_url();
	...
	var dataTable = $('#table-grid').DataTable({
				...
				"ajax":{
					type: "post", 
					url :base_url+"Main_sales/so_item_onlineConvertDetails", // json datasource
					data:{'olsono':olsono},
...
$(".btnConvert").click(function(e){
		...
		if(drno_id > 0)
		{
			...
			$.ajax({
			  		type: 'post',
			  		url: base_url+'Main_sales/save_item_saleonlineDetails',
			  		data:{'olsono':olsono
			  		},
			  		beforeSend:function(data){
			  		...
			  		success:function(data)
			  		{
			  			if (data.success == 1) 
			  			{
			  				...
			  				$(".btnDeliveryComfirm").text("Converted DR");
								$.toast({
								    heading: 'Success',
								    text: "OLSO #"+ olsono +" has been successfully converted to delivery receipt.",
								    icon: 'success',
								    loader: false,  
								    stack: false,
								    position: 'top-center', 
							...
							window.setTimeout(function(){
								window.location.href=base_url+"Main_sales/sales_online/" + token;
							},500)
