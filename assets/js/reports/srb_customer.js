$(document).ready(function(){
	...
	// 	window.location.href = base_url+"Main_reports/icr_by_item_print/"+datefrom+"/"+dateto+"/"+category+"/"+location;
	...
	$(".btnSearchSO").click(function(e){
		...
		{
			if(date1 == "" && date2 == "" && location == "none")
			{
				checker=0;
				$.toast({
				    heading: 'Note:',
				    text: "Please fill date range field and please select Customer.",
				    icon: 'info',
				    loader: false,   
				    stack: false,
				    position: 'top-center',  
			...
			else if(date1 != "" && date2 != "" && location == "none")
			{
				checker=0;
				$.toast({
				    heading: 'Note:',
		...
		if(checker == 1)
		{
			...
			dataTable = $('#table-grid').DataTable({
				...
				"serverSide": true,
				"ajax":{
					url:base_url+"Main_reports/srb_customer_table", // json datasource
