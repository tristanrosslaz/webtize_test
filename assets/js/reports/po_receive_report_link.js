$(document).ready(function(){
	...
	dataTable = $('#table-grid2').DataTable({
		...
		"serverSide": true,		
		"ajax":{
			url:base_url+"Main_reports/get_poreceive_log", // json datasource
	...
	$('#table-grid2').delegate(".itemid", "click", function(){
	  	...
	  	dataTable2 = $('#table-grid').DataTable({
			...
			"serverSide": true,		
			"ajax":{
				url:base_url+"Main_reports/get_poreceive_breakdown", // json datasource
				type: "post",  // method  , by default get
				data:{"rcvno": rcvno, "itemid": itemid},
				error: function(){  // error handling
