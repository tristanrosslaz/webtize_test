$(function(){
	...
	$("#searchBtn").click(function(){
		...
		var filter_report2 = $(".filter_report2").val();
		//alert($(".loader").text());
		if (filter_report == 1) { //with details
			...
			var i2 = $(".searchDate2").attr('data-column'); 
			var v2 = $(".searchDate2").val();
			// 
			count++;
			$.ajax({
				type:'post',
				url:base_url+'Main_reports_salesandcollection/bankrecon_report_getshowtd',
				data:{'v':v, 'v2':v2, 'filter_report2':filter_report2},
				...
				success:function(data){
					if (data.success == 1) {
						...
						// balik123
						table_bank(i, v, i2, v2, filter_report, filter_report2);
						// $(".dynamic_card").show();
	...
	function table_bank(i, v, i2, v2, filter_report, filter_report2){
		...
		if (filter_report == 1) { //with details
			var dataTable = $('#table-grid').DataTable({
				...
				"destroy": true,
				"ajax":{
					url :base_url+"Main_reports_salesandcollection/bankrecon_report_table", // json datasource
					type: "post",  // method  , by default get
					beforeSend:function(data){
						$("table#table-grid").css("background", "#0000000d");
					},
					data:{'trandate':v, 'trandate2':v2, 'filter_report2':filter_report2},
		        ...
		        "fnDrawCallback": function() {
					...
					//balik 091218
			        gettableHeader(v, v2, filter_report, filter_report2); //trigger 2nd and 3rd table
					...
					$("table#table-grid").css("background", "none");
	...
	function gettableHeader(trandate, trandate2, filter_report, filter_report2){ //v, v2, filter_report
		if (filter_report == 1) { //with details
			var total_deposit = 0;
			var grand_discrepancy = 0;
			$.ajax({
				type:'post',
				url:base_url+'Main_reports_salesandcollection/getRecontableHeader',
				data:{'trandate':trandate, 'trandate2':trandate2},
				success:function(data){
					if (data.success == 1) {
						var res = data.result;
						var reslen = res.length;
