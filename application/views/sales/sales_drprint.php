...
<p class="line-height-two">Contact #: <?php echo company_phone(); ?></p>
  ...
  <div class="container-fluid main-content">
    ...
    <p class="line-height-one"><?=today()?></p>
     ...
     <h2 class="text-right" style="line-height: 1px;">#<?=$getSummary->drno;?></h2>
       <p class="text-right" style="line-height: 2px; "><?=$getSummary->trandate?></p>
       <p class="line-height-one text-right"><?php echo $getSummary->ispaid ?></p>
       
        <h5>Delivery Information</h5>
        <p class="line-height-one">Encoded by: <?=get_name_by_username($getSummary->username) ?></p>
          <p class="line-height-one">Sold To: <?php echo strtoupper($getSummary->lname . ", " . $getSummary->fname . " " . $getSummary->mname)?></p>
    ...
    <table class="table table-striped table-hover table-bordered"  cellpadding="0" cellspacing="0" border="0" class="display" width="100%">
        <thead>
           <tr>
              ...
              <th class="text-right" width="75px">Discount</th>
              <th class="text-right" width="75px">Total</th>
           </tr>
        </thead>
        <tbody>
          <?php     
            $query1      = $this->model_sales->get_dr_Details($drno);
            foreach ($query1->result_array() as $row) {
                ...
                if($row["discamt"] == "0" || $row["discamt"] == NULL) {
                  echo
                    '<tr>
                      <td width="150px">' . strtoupper($item_name->itemname) .  '</td>
                      <td width="75px">'.number_format($row["qty"], 2).'</td>
                      <td width="75px">'.$item_unit->description.'</td>
                      <td width="75px">'.number_format($row["price"], 2).'</td>
                      <td class="text-right" width="75px"></td>
                      <td class="text-right" width="75px">'.number_format($grand_total, 2).'</td>
                    </tr>'; 
                }
                else {
                  if ($row['discount_type'] == 2) { //percentage
                    ...
                    '<tr>
                        ...
                        <td width="75px">'.$item_unit->description.'</td>
                        <td width="75px">'.number_format($row["price"], 2).'</td>
                        <td class="text-right" width="75px">'.$row["discamt"].'%</td>
                        <td class="text-right" width="75px">'.number_format($grand_total, 2).'</td>
                     </tr>'; 
                  }
                  else { //whole number
                    echo
                     '<tr>
                        ...
                        <td width="75px">'.$item_unit->description.'</td>
                        <td width="75px">'.number_format($row["price"], 2).'</td>
                        <td class="text-right" width="75px">'.number_format($row["discamt"], 2).'</td>
                        <td class="text-right" width="75px">'.number_format($grand_total, 2).'</td>
                     </tr>'; 
                  }
                }
        ...
        <tfoot>
          ...
          <?php } ?>
            ...
            <tr>
              <td class="text-right" colspan="5">
                <h3 style="line-height: 4px;">Total:  </h3>
              </td>
              <td class="text-right">
                <h3 style="line-height: 4px;"><?=$total_amount_computed;?></h3>
              </td>
            </tr>
          </tfoot>
    </table>
    <div style="margin-top: 10px; width: 100%" class="text center">
      <p >I hereby certify that I receive the items in good condition and complete quantity</p>
